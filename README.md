...
| Error | Root Cause                                                                                                                                                                                                           | Potential Solution                                                                                                                                                                                                                                                                         |
|---|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `FirstScan`/`SecondScan response must always return a non error status code!` | One of the two baseline logins (with valid credentials) returned an HTTP status > 399. The app itself is failing before any scanner runs.                                                                            | Verify the target app is healthy and the configured credentials/config are valid; try reproduce the login manually.                                                                                                                                                                        |
| `Third scan must always be classified as OK!` | The control scan (deliberately failing login) was classified as `VULNERABLE` instead of `OK`. Verification strategies can't distinguish a failed login from a successful one.                                        | Check which `VerificationStrategy` matched and why; adjust `verificationStrategies` in the scanner config.                                                                                                                                                                                 |
| `Fourth scan must always be classified as VULNERABLE!` | The control scan (valid successful login, no manipulation) was classified as `OK` instead of `VULNERABLE`. There's no measurable drift between the baseline and this scan, so real attacks can't be detected either. | Same as above — pick/adjust verification strategies that actually detect a difference for this app.                                                                                                                                                                                        |
| `Privacy error` | Chrome navigated to a TLS/certificate error page (e.g. self-signed or expired cert) instead of the expected page.                                                                                                    | Fix the applications SSL cert                                                                                                                                                                                                                                                              |
| `Timeout waiting for captured request!` | The federation request (SAML or OIDC request) never showed up within `pageLoadTimeoutInSeconds`.                                                                                                                     | Inspect the attached screenshot/traffic log/visible text to see where the app is stuck. Use either a deeplink to start the login or leverage selenium actions|
| `Reachability check failed` | TCP connect to the target host:port failed before the browser even started navigating. For instance unresolved host, connection refused, no route, or connect timeout.                                               | Check DNS, confirm the service is up and listening, check network/firewall/VPN connectivity from where SAFED runs.                                                                                                                                                     |
