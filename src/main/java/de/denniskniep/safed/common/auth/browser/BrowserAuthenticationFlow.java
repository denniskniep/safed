...
public abstract class BrowserAuthenticationFlow<T> extends Browser  {
    ...
    protected BrowserAuthenticationFlow() {
    ...
    public T initialize(URL relyingPartySignInUrl) {
        ...
        var page = execute(httpRequest, this::isRequestToIdp);
    ...
    public Page answerWith(HttpRequest httpRequest) {
        ...
        var page = execute(httpRequest, r -> StringUtils.equalsIgnoreCase(r.getMethod(), httpRequest.method()) && StringUtils.equalsIgnoreCase(r.getUrl(), httpRequest.url()));
